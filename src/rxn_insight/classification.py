import itertools
from src.rxn_insight.utils import *
...
class ReactionClassifier:
    ...
    def __init__(self, reaction: str, rxn_mapper=None, keep_mapping=False):
        if keep_mapping:
            self.mapped_reaction = reaction
            self.reaction = remove_atom_mapping(reaction)  # Remove atom mapping for consistency
        else:
            self.reaction = reaction
            self.mapped_reaction = get_atom_mapping(self.reaction, rxn_mapper=rxn_mapper)
        self.sanitized_mapped_reaction, self.sanitized_reaction, \
            self.extra_agents = sanitize_mapped_reaction(self.mapped_reaction)
        self.template = get_reaction_template(self.sanitized_mapped_reaction,
                                              radius_reactants=1,
                                              radius_products=0)
        self.template_smiles = self.get_template_smiles()
        self.reactants, self.products = self.sanitized_mapped_reaction.split(">>")
        self.reactant_mols = tuple([Chem.MolFromSmiles(mol) for mol in self.reactants.split(".")])
        self.product_mols = tuple([Chem.MolFromSmiles(mol) for mol in self.products.split(".")])
        ...
        self.reaction_center_idx = [self.product_map_dict[i] for i in self.sanitized_transformation_mapping
                                    if i in self.product_map_dict]
        self.transformation_determinant = np.linalg.det(self.sanitized_transformation_matrix)
    ...
    def get_template_smiles(self) -> str:
        """
        ...
        for j in range(len(reactants)):
            ...
            for g in range(len(reactants_template)):
                ...
                if len(reactant_molecule.GetSubstructMatch(reactant_template)) > 0:
                    atoms_reactant.append(reactant_molecule.GetSubstructMatch(reactant_template))
                    break
        ...
        for k in range(len(products)):
            ...
            for g in range(len(products_template)):
                ...
                if len(product_molecule.GetSubstructMatch(product_template)) > 0:
                    atoms_products.append(product_molecule.GetSubstructMatch(product_template))
                    break
        ...
        rxn = ".".join([Chem.MolToSmiles(r) for r in subreactants]) + ">>" + Chem.MolToSmiles(subproducts[0])

        return rxn

    def get_functional_group_smarts(self, molecule: Mol, matrix: np.ndarray, map_dict: dict) -> tuple:
        ...
        for i in range(len(matrix_indices)):
            ...
            if self.atoms_diagonal[idx] == 6 and not np.all(self.atoms_diagonal[matrix_indices[i:]] == 0):
    ...
    def get_functional_groups(self, mol: Mol, map_dict: dict, df: pd.DataFrame) -> tuple:
        maps = self.transformation_mapping
        atom_indices = np.array([map_dict[atom_map] for atom_map in maps if atom_map in map_dict])
        fg = []
        visited_atoms = []
        for i in df.index:
    ...
    def get_ring_type(self, mol: Mol, map_dict: dict = None):
        try:
        ...
        if map_dict is not None:
            if len(rs) == 0:
                ...
                for r in rs:
                    r = np.array(list(r))
                    if len(r[np.in1d(r, atom_indices)]) != 0:
    ...
    def balance_reaction(self, fgr: tuple, fgp: tuple):
        ...
        negative_values = np.unique(np.array(list(negative_values) + list(metal_indices)))
        ...
        try:
            lost_all = Chem.AddHs(self.mol_reactant).GetNumAtoms() - Chem.AddHs(self.mol_product).GetNumAtoms()
        ...
        for x in [Chem.GetPeriodicTable().GetElementSymbol(int(symbol)) for symbol in metals]:
            if x not in lost_dict:
                continue
            else:
        ...
        return tuple(small_molecules)

    def get_reaction_center_info(self, df: pd.DataFrame):
        reaction_center = dict()
        ...
        reaction_center["FG_REACTANTS"] = self.get_functional_groups(self.mol_reactant, self.reactant_map_dict, df)
        reaction_center["FG_PRODUCTS"] = self.get_functional_groups(self.mol_product, self.product_map_dict, df)
        reaction_center["PARTICIPATING_RINGS_REACTANTS"] = self.get_ring_type(self.mol_reactant, self.reactant_map_dict)
        reaction_center["PARTICIPATING_RINGS_PRODUCTS"] = self.get_ring_type(self.mol_product, self.product_map_dict)
        reaction_center["ALL_RINGS_PRODUCTS"] = self.get_ring_type(self.mol_product)
        reaction_center["BY-PRODUCTS"] = self.balance_reaction(reaction_center["FG_REACTANTS"],
                                                               reaction_center["FG_PRODUCTS"])
        reaction_center["CLASS"] = self.classify_reaction()
        reaction_center["TAG"] = tag_reaction(reaction_center)
        return reaction_center

    def get_atom_mapping_indices(self) -> (dict, np.ndarray, int):
        """
    ...
    def get_be_matrix(self, molecule: Mol) -> np.ndarray:
        """
        ...
        for atom in molecule.GetAtoms():
            ...
            matrix[idx][idx] = Chem.GetPeriodicTable().GetNOuterElecs(atom.GetAtomicNum()) - atom.GetTotalValence()
    ...
    def sanitize_r_matrix(self) -> (np.ndarray, np.ndarray, np.ndarray):
        """
        ...
        clean_r_matrix = clean_r_matrix[atoms_to_remove][:, ~np.all(clean_r_matrix[atoms_to_remove] == 0.0, axis=0)]
    ...
    def remove_metals_and_halogens(self) -> (np.ndarray, np.ndarray, np.ndarray):
        """
        ...
        for i in range(len(self.reaction_center_atoms)):
            if self.reaction_center_atoms[i] in ignore_atoms and self.transformation_matrix.diagonal()[i] < 0:
                if i not in to_delete:
                    self.removed_halogens = True
                    to_delete.append(i)
            elif self.reaction_center_atoms[i] in metals and \
                    len(np.where(np.array(self.transformation_matrix[:, i]) < 0)[0]) > 0:
                if i not in to_delete:
        ...
        for j in reversed(range(len(to_delete))):
            clean_transformation_matrix = np.delete(clean_transformation_matrix, (to_delete[j]), axis=0)
            clean_transformation_matrix = np.delete(clean_transformation_matrix, (to_delete[j]), axis=1)
    ...
    def check_nos(self) -> bool:
        ...
        for i in range(len(self.sanitized_reaction_center)):
            ...
            if atom in nos_atoms and len(np.where(self.sanitized_transformation_matrix[:, i] > 0)[0]) > 0:
                nos = True
                break
    ...
    def ring_changing(self) -> int:
        ...
        ri_r = reactants.GetRingInfo().NumRings()
        ri_p = products.GetRingInfo().NumRings()
    ...
    def is_fgi(self):
        ...
        if len(self.sanitized_transformation_matrix) == 1 and m[0][0] == 0:
            return True
        d = m.diagonal()
        a = np.array([self.atom_mapping_index[atom_map] for atom_map in self.transformation_mapping])
        if len(a) == 0:
            return False
        ...
        x_indices = np.array(list(np.where(involved_types == 9)[0])
        ...
        for idx in indices:
            if sum(m[:, idx]) != 0:
                if len(np.where(d > 0)[0]) == len(np.where(d < 0)[0]) \
                        and len(np.where(m[:, idx] > 0)[0]) == len(np.where(m[:, idx] < 0)[0]):
                    return True
                otf = "[O;D2;+0]-[S;D4;+0](=[O;H0;D1;+0])(=[O;H0;D1;+0])-[CX4;D4;+0](-F)(-F)-F"
                ots = "[C;H3;D1;+0]-[c;H0;D3;+0]1:[c;H1;D2;+0]:[c;H1;D2;+0]:[c;H0;D3;+0]" \
                ...
                leaving = [Chem.MolFromSmarts(otf), Chem.MolFromSmarts(ots), Chem.MolFromSmarts(oms),
                           Chem.MolFromSmarts(thiocyanate), Chem.MolFromSmarts(isothiocyanate),
                           Chem.MolFromSmarts(isocyanate), Chem.MolFromSmarts(isocyanide)]
                for group in leaving:
                    if len(self.mol_reactant.GetSubstructMatches(group)) > 0 \
                            and len(self.mol_product.GetSubstructMatches(group)) == 0:
                        if self.is_cc_coupling() or self.is_heteroatom_alkylation():
                            return False
                        else:
                            return True
                    elif len(self.mol_reactant.GetSubstructMatches(group)) > 0 \
                            and len(self.mol_product.GetSubstructMatches(group)) > 0:
                        if len(a[np.in1d(a, self.mol_product.GetSubstructMatches(group))]) > 0:
                            return True
                        else:
                            continue
                    else:
                        continue
                if len(np.where(d > 0)[0]) == 0 and len(np.where(d < 0)[0]) > 0 and len(
                        np.where(m[:, idx] < 0)[0]) == 0 and self.num_reactants == 1:
                    return True
                elif len(np.where(d < 0)[0]) == 0 and len(np.where(d > 0)[0]) > 0 and len(
                        np.where(m[:, idx] > 0)[0]) == 0 and self.num_reactants == 1 and not self.is_oxidation():
                    return True
                elif len(np.where(d > 0)[0]) == 0 and len(np.where(d < 0)[0]) > 0 and self.num_reactants == 2 and \
                        len(x_indices) > 0:
                    for x_idx in x_indices:
                        if len(np.where(m[:, x_idx] < 0)[0]) == len(np.where(m[:, x_idx] > 0)[0]):
                            return True
                else:
                    boronic = Chem.MolFromSmarts("[#6]-[BX3]")
                    nitro = Chem.MolFromSmarts("[$([NX3](=O)=O),$([NX3+](=O)[O-])][!#8]")
                    if len(self.mol_product.GetSubstructMatches(boronic)) > 0 and \
                            len(self.mol_reactant.GetSubstructMatches(boronic)) == 0:
                        return True
                    elif len(self.mol_product.GetSubstructMatches(nitro)) == 0 and \
                            0 < len(self.mol_reactant.GetSubstructMatches(nitro)) < len(np.where(m > 0)[0]) and \
        ...
        if d.any():
            nitro = Chem.MolFromSmarts("[$([NX3](=O)=O),$([NX3+](=O)[O-])][!#8]")
            if len(self.mol_product.GetSubstructMatches(nitro)) == 0 and \
                    0 < len(self.mol_reactant.GetSubstructMatches(nitro)):
            ...
            if len(np.where(d > 0)[0]) == len(np.where(d < 0)[0]):
                ...
                if len(s_indices) > 0 and len(o_indices) > 0:
                    ...
                    return True
            elif len(np.where(d < 0)[0]) > 0 and len(np.where(m[:, si_indices] == 1)[0]) > 0:
    ...
    def is_aromatic_heterocycle(self):
        aromatic_changes = np.array(list(np.where(self.sanitized_transformation_matrix == 1.5)[0]) +
        ...
        if len(aromatic_changes) == 0:
            ...
            if more_rings and self.nos_reaction_center:
                ...
                if len(a) == 0:
                    return False
                aromatic_bonds_products = len(np.where(self.be_matrix_products[:, a] == 1.5)[0])
                aromatic_bonds_reactants = len(np.where(self.be_matrix_reactants[:, a] == 1.5)[0])
    ...
    def is_reduction(self):
        if self.num_reactants == 1:
            m = self.transformation_matrix
            only_nonpositive = len(np.where(self.sanitized_transformation_matrix > 0)[0]) == 0
            a = np.array([self.atom_mapping_index[atom_map] for atom_map in self.transformation_mapping])
            if len(a) == 0:
                return False
            involved_types = self.atoms_diagonal[a]
            ...
            if not only_nonpositive:
                ...
                if len(np.where(m == -0.5)[0]) > 0 and len(np.where(m == 0.5)[0]) > 0 and len(np.where(m == -1)[0]) > 0:
                    return True
                elif len(s_indices) > 0 and len(o_indices) > 0:
                    if len(np.where(m.diagonal()[s_indices] == 4)[0]) > 0 \
                            and len(np.where(m.diagonal()[o_indices] == -4)[0]) > 0:
                        return True
                    else:
                        return False
                elif len(n_indices) > 0 and len(o_indices) > 0:  # Oxime reduction
                    if len(np.where(m.diagonal()[n_indices] == -2)[0]) > 0 \
                            and len(np.where(m.diagonal()[o_indices] == 0)[0]) > 0 \
                            and len(np.where(m[n_indices] >= 0)[0]) == 0:
                        return True
                    elif 0 < len(np.where(m.diagonal()[o_indices] == -5)[0]) == len(np.where(m > 0)[0]) \
                    ...
                    if len(np.where(self.transformation_matrix.diagonal()[n_indices] == 1)[0]) == 0:
                        return False
                if len(np.where(self.sanitized_transformation_matrix > 0)[0]) > len(
                        np.where(self.sanitized_transformation_matrix.diagonal()[n_indices] == 1)[0]):
                    return False
            ...
            carbon_balance_reactants = len(all_reactant_mappings[np.in1d(all_reactant_mappings, carbons)]) == len(
                carbons)
            carbon_balance_products = len(all_product_mappings[np.in1d(all_product_mappings, carbons)]) == len(carbons)
            if carbon_balance_products and carbon_balance_reactants:
                return True
            elif len(all_reactant_mappings[np.in1d(all_reactant_mappings, oxygens)]) > len(
                    all_product_mappings[np.in1d(all_product_mappings, oxygens)]):
                return True
            elif len(all_reactant_mappings[np.in1d(all_reactant_mappings, sulfurs)]) > len(
                    all_product_mappings[np.in1d(all_product_mappings, sulfurs)]):
                return True
            else:
                return False
        else:
        ...
        if self.num_reactants == 1:
            ...
            if negative:
                ...
                if len(np.where(m == -0.5)[0]) > 0 and len(np.where(m == 0.5)[0]) > 0 and len(np.where(m == 1)[0]) > 0:
                    ...
                    if len(np.where(m.diagonal()[s_indices] == -4)[0]) > 0 \
                            and len(np.where(m.diagonal()[o_indices] == 4)[0]) > 0:
                        return True
                    elif len(np.where(m.diagonal()[o_indices] == 4)[0]) > 0 and len(
                            np.where(m.diagonal()[s_indices] == -2)[0]) > 0:
                        return True
                    else:
                        return False
                    ...
                    try:
                        if len(np.where(m[c_indices, o_indices] < 0)[0]) == 0 \
                                and len(np.where(m.diagonal()[o_indices] == 4)[0]) > 0:
                            return True
                    except IndexError:
                        return False
                    ...
                    if len(np.where(m.diagonal()[n_indices] == -1)[0]) == 0:
                        return False
                if len(np.where(m < 0)[0]) > len(np.where(m.diagonal()[n_indices] == 1)[0]):
                    return False
            d = m.diagonal()
            zero_diagonal = ~d.any()
            ...
            if negative:
                ...
                for sulfur in sulfurs:
                    if len(np.where(self.be_matrix_products[sulfur, a[o_indices]] == 2)[0]) > len(
    ...
    def is_acylation(self):
        if self.num_reactants == 1:
                    ...
                    for tup in ester_reactant:
                        ...
                        for tup in ester_products:
                            ...
                            if len(carboxyl_in_rcr) > 0 and len(carboxyl_in_rcp) > 0:
                                if len(np.intersect1d(carboxyl_in_rcr, carboxyl_in_rcp)) > 0:
                                    return False
            ...
            x_indices = np.array(list(np.where(self.atoms_diagonal == 9)[0])
            ...
            if len(s_indices) > 0 and not self.is_oxidation():
                sulfurs = a[s_indices]
                for sulfur in sulfurs:
                    sulfone_r = np.where(self.be_matrix_reactants[sulfur, o_indices] == 0)[0]
                    sulfone_p = np.where(self.be_matrix_products[sulfur, o_indices] == 2)[0]
                    if len(sulfone_r) > 0 and len(sulfone_p) > 0:
                        if len(np.intersect1d(sulfone_r, sulfone_p)) > 0:
                            return True
                        elif len(x_indices) > 0:
                            if len(np.where(self.be_matrix_reactants[sulfur, x_indices] == 1)[0]) > 0 and \
                                    len(np.where(self.be_matrix_products[sulfur, x_indices] == 1)[0]) == 0:
                                return True
            ...
            if acyl and carbonyl_c != -1:
                ...
                if 7 in bond_formed_with or 8 in bond_formed_with or 16 in bond_formed_with:
                    return True
    ...
    def is_heteroatom_alkylation(self):
        if not self.nos_reaction_center:
            ...
            if len(a) == 0:
                return False
            involved_types = self.atoms_diagonal[a]
            nos_indices = list(np.where(involved_types == 7)[0]) + list(np.where(involved_types == 8)[0]) + list(
                np.where(involved_types == 16)[0])
            ...
            for heteroatom in heteroatoms:
                maps_1 = np.array(list(np.where(self.r_matrix[:, heteroatom] == 1)[0])
                                  + list(np.where(self.r_matrix[:, heteroatom] == 2)[0]))
                if len(maps_1) == 0:
                    continue
                else:
                    no_bonds = np.where(self.be_matrix_reactants[maps_1, heteroatom] == 0)[0]
                    ...
                    if len(o_indices) == 0:
                        ...
                        for carbon in carbons:
                            carbonyls_r += list(np.where(self.be_matrix_reactants[carbon, o_indices] == 2)[0])
                            carbonyls_p += list(np.where(self.be_matrix_products[carbon, o_indices] == 2)[0])
                    if len(carbon_bonds) > 0 and len(no_bonds) > 0:
                        if len(np.intersect1d(maps_1[no_bonds], carbon_bonds)) > 0 \
                                and not (len(carbonyls_r) > 0 and len(carbonyls_p) > 0):
                            return True
                        else:
                            return False
                    else:
    ...
    def is_cc_coupling(self):
        if self.num_reactants == 1 and self.ring_change <= 0:
            ...
            carbons = a[c_indices]
            for carbon in carbons:
                maps_1 = np.array(list(np.where(self.r_matrix[:, carbon] == 1)[0])
                                  + list(np.where(self.r_matrix[:, carbon] == 2)[0]))
                if len(maps_1) == 0:
                    continue
                else:
                    no_bonds = np.where(self.be_matrix_reactants[maps_1, carbon] == 0)[0]
    ...
    def is_fga(self):
        ...
        if len(np.where(d < 0)[0]) > 0:
            ...
            if count_x_additions > 0 and 2 * count_x_additions == len(np.where(m > 0)[0]):
                return True

            si_indices = np.where(involved_types == 14)[0]
            for si in si_indices:
                if len(np.where(m[:, si] == 1)[0]) > 0 and len(np.where(sm.diagonal() < 0)[0]) == 0:
                    if not self.is_protection():
                        return True
                    else:
                        return False
            ...
            if len(self.mol_reactant.GetSubstructMatches(nitro)) > 0 and \
                    len(self.mol_product.GetSubstructMatches(nitro)) > 0 and \
                    len(n_indices) > 0 and len(c_indices) > 0 and self.num_reactants == 2:
                for n in n_indices:
    ...
    def is_deprotection(self):
        if self.num_reactants == 1:
            ...
            nos_indices = list(np.where(involved_types == 7)[0]) + list(np.where(involved_types == 8)[0]) + list(
                np.where(involved_types == 16)[0])
            c_si_indices = list(np.where(involved_types == 6)[0]) + list(np.where(involved_types == 14)[0])
            if len(nos_indices) == 0:
                alkyne_indices = []
            ...
            for heteroatom in heteroatoms:
                ...
                if len(maps_1) == 0:
                    ...
                    no_bonds = len(np.where(self.be_matrix_products[maps_1, heteroatom] == 0)[0])
                    if len(maps_1[np.in1d(maps_1, carbons)]) > 0 and no_bonds > 0:
                        in_products = len(np.where(self.be_matrix_products[maps_1[np.in1d(maps_1, carbons)]] != 0)[0])
                        if in_products > 0:
                            continue
                        else:
                            return True
        ...
        if self.num_reactants == 1:
            ...
            for heteroatom in heteroatoms:
                ...
                if len(maps_1) == 0:
                    ...
                    no_bonds = len(np.where(self.be_matrix_reactants[maps_1, heteroatom] == 0)[0])
                    if len(maps_1[np.in1d(maps_1, carbons)]) > 0 and no_bonds > 0:
                        return True
                    else:
                        continue
    ...
    def name_reaction(self, smirks_db: pd.DataFrame) -> str:
        ...
        if len(reactants) > 4 or len(products) > 4:  # There are no templates for reactions with more than four reactants.
        ...
        for product in products:
            try:
                new_products.append(Chem.MolToSmiles(Chem.MolFromSmiles(product), isomericSmiles=False))
            except:
        ...
        if num_reactants == 1:
            ...
            all_tuples = list(itertools.permutations(react_tuple))  # RDKit does not permute reactants by itself
        ...
        for i in selected_rxns.index:  # Iterate over all reactants to find a match
            ...
            if len(pred_products) == 0:  # No products are found
                ...
                for prods in pred_products:
                    ...
                    if prod in new_products:  # Predicted product is in the real reaction
